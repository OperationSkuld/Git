...
function changeaddress(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(c[i]).selectedIndex-1));
	};
	var ss=document.getElementById(c[v]);
	with(ss){
		length = 0;
		options[0]=new Option(opt[v],opt[v]);
		if(v && document.getElementById(c[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
...
var c=["change_province","change_city","change_county"];//三个select的name
var opt = ["选择省","选择市","市、县级市"];//初始值
function _change_area(){  //初始化函数
	for(i=0;i<c.length-1;i++){
	  document.getElementById(c[i]).onchange=new Function("changeaddress("+(i+1)+")");
	}
	changeaddress(0);
}

var d=["province","city","county"];//三个select的name
var opt1 = ["选择省","选择市","市、县级市"];//初始值
function _select_area(){  //初始化函数
	for(i=0;i<d.length-1;i++){
	  document.getElementById(d[i]).onchange=new Function("selectaddress("+(i+1)+")");
	}
	selectaddress(0);
}